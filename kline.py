...
class Kline:
    ...
    def session_distribute(self):
        ...
        for index, session in enumerate(self.sessions):
            ...
            limiter_config = {
                ...
                "cmp_rate": self.cmp_rate,
                "priority": priority_symbols,
                "common": common_symbols,
            }
            if hasattr(self, "second_limit"):
                limiter_config["second_limit"] = self.second_limit 
            rate_limiter = PriorityRateLimiter(**limiter_config)
            self.clients.append(rate_limiter)
    ...
    def get_coinpair(self):
        datas = []
        try:
            r = requests.post("https://galaxy.sandyvip.com/api/coinpair/",
    ...
    async def worker(self, client, symbol):
        success_count = 0
        fail_count = 0
        average = 0
        req_count = 0
        ...
        for cycle_count in range(self.cycles):
    ...
    def params_prepare(self, market, intervals): 
        ...
        for interval in intervals:
            ...
            if self.name == "Poloniex":
                ...
                url = filled_params['url'].format(start = (int(final_time) / 1000))
            else:
                url = filled_params['url']

            # 根据之前完善好的请求配置信息填写请求表单信息
            fetch_kwargs = {}
            for _property in ("_json", "data", "params"):
                if _property in filled_params:
                    fetch_kwargs[_property] = filled_params[_property]
            fetch_kwargs['url'] = url
            fetch_kwargs['symbol'] = market
    ...
    async def fetch(self, session, url, data=None, _json=None, params=None, symbol=None):
        try:
            if params:
            ...
            async with await session_method(url, timeout=timeout, ssl=False,
                data=data, json=_json, params=params, headers=self.headers, symbol=symbol) as response:
    ...
    def parse_data(self, mongo_key, crude_data):
        try:
            # 提取数据
            data_list = self.get_data(crude_data) if self.get_data else crude_data
            ...
            try:
                ...
                traceback.print_exc()
                self.logging.error('[Kline {} Saving Error]: {}'.foramt(mongo_key, sub_err))

        except Exception as err:
            traceback.print_exc()
            self.logging.error('[Kline {} Parsing Error]: {}'.foramt(mongo_key, err))
    ...
    def data_save(self, redis_keys, mongo_key, data_list, unique_bar_times):
        ...
        if mongo_col.find_one():
            last_doc_dict = mongo_col.find({}, {"bar_time": 1, "_id": 0}).sort("$natural", -1).limit(1)[0]
            last_mongo_time = last_doc_dict.get('bar_time')
            if last_mongo_time in recent_bar_times:
                mongo_col.remove({"bar_time": {"$gte": last_mongo_time}})
        ...
        try:
            mongo_datas = mongo_col.find({}, {"bar_time": 1, "_id": 0}).sort("$natural", -1)
            mongo_col.insert_many(mongo_insert_list)
    ...
    def get_klines(self):
        ...
        for client in self.clients:
            priority_symbols = client.priority_symbols
            common_symbols = client.common_symbols
            for symbol in priority_symbols + common_symbols:
                tasks.append(self.worker(client, symbol))
    ...
    def query_final_time(self, collection):
        # 获取Mongo某一Collection之前最大的时间戳
        try:
            last_doc = self.db[collection].find(
                {}, {"bar_time": 1, "_id": 0}).sort("_id", -1).limit(1)
            last_bt = last_doc[0].get('bar_time')
            return last_bt
        except Exception as e:
            self.logging.info("[Function: query_final_time] Error: %s" % e)
            # 初始 2018-08-01 00:00:00
            return 1533052800000
